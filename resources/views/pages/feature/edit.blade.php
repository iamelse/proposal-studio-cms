...
<main>
    <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
        ...
        <!-- Form Section -->
        <div x-data="slugGenerator('{{ old('title', $feature->title ?? '') }}', '{{ old('slug', $feature->slug ?? '') }}')" class="border-gray-100 p-5 dark:border-gray-800 sm:p-6">
            <div class="rounded-2xl px-6 pb-8 pt-4 border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <form action="{{ route('be.feature.update', $feature->slug) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mt-4">
                        ...
                        @if (!empty($feature->image))
                            <div class="mt-3">
                                ...
                                <img src="{{ getWhyUsListImagePath($feature) }}" alt="Current Image" class="h-24 rounded border">
                            </div>
                        @endif
                    </div>
            ...
            return {
                ...
                updateSlug() {
                    if (this.title.length > 0) {
                        fetch("{{ route('be.feature.generate.slug') }}", {
                            method: "POST",
                            headers: {
