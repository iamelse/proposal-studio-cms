...
<main>
   <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
    ...
    <div class="flex px-6 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Review Management</h1>
            <p class="text-gray-600 dark:text-gray-400">Manage review data</p>
        </div>
        @can(PermissionEnum::CREATE_REVIEW, $reviews)
            <a href="{{ route('be.review.create') }}"
    ...
    <div class="border-gray-100 p-5 dark:border-gray-800 sm:p-6" x-data="{ selected: [] }">
        <div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="mb-4 flex flex-col gap-2 px-5 sm:flex-row sm:items-end sm:justify-end sm:px-6">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div class="relative flex items-center gap-2">
                        ...
                        <div x-data="{ openReviewMassDeleteModal: false, deleteUrl: '' }">
                            <!-- Delete Selected Button -->
                            <a href="#"
                                x-on:click.prevent="
                                    if (selected.length > 0) {
                                        let params = new URLSearchParams({ ids: selected.join(',') });
                                        deleteUrl = '{{ route('be.review.mass.destroy') }}?' + params.toString();
                                        openReviewMassDeleteModal = true;
                                    }
                                "
                            ...
                            <div x-show="openReviewMassDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-[400px]">
                                    ...
                                    <div class="mt-4 flex justify-end gap-3">
                                        <button @click="openReviewMassDeleteModal = false"
                        ...
                        <div x-data="{ open: false, selectedField: '{{ request()->query('filter') ? array_key_first(request('filter')) : '' }}' }">
                            ...
                            <div x-cloak x-show="open" @keydown.escape.window="open = false"
                                ...
                                <div @click.away="open = false"
                                    ...
                                    <!-- Form -->
                                    <form method="GET" action="{{ route('be.review.index') }}">
                                        <!-- Limit Selection -->
                                        <div class="mt-4">
                        ...
                        <tr class="text-left text-gray-600 dark:text-gray-300 text-sm">
                            <th class="w-10 px-6 py-3">
                                <input
                                    ...
                                    x-bind:checked="selected.length > 0 && selected.length === document.querySelectorAll('.review-checkbox').length"
                                    x-on:change="selected = $event.target.checked ?
                                        [...document.querySelectorAll('.review-checkbox')].map(cb => cb.value) : []">
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800 dark:text-gray-400">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                            <td class="w-10 px-6 py-3">
                                <input
                                    type="checkbox"
                                    class="review-checkbox flex h-5 w-5 border-gray-300 cursor-pointer items-center justify-center rounded-md border-[1.25px] transition-all" value="{{ $review->id }}"
                                    x-model="selected">
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-col items-center">
                                    <div class="p-2 sm:p-3 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center w-12 sm:w-16 h-12 sm:h-16">
                                        <img class="rounded-full" src="{{ $review->image }}" alt="{{ $review->name }}">
                                    </div>
                                    <span class="mt-2 text-xs font-medium text-gray-500 dark:text-gray-300">{{ $review->name }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 text-center relative">
                                <div x-cloak x-data="{ openDropDown: false }" class="inline-block">
                                    ...
                                    <div x-show="openDropDown" @click.outside="openDropDown = false"
                                        ...
                                        @can(PermissionEnum::UPDATE_REVIEW, $review)
                                        <a href="{{ route('be.review.edit', $review->id) }}" class="block w-full px-4 py-2 text-left text-sm text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">
                                            Edit
                                        </a>
                                        @endcan
                                        <!-- Alpine.js State Wrapper -->
                                        <div x-data="{ openReviewDeleteModal: false }">
                                            <!-- Delete Button -->
                                            @can(PermissionEnum::DELETE_REVIEW, $review)
                                            <button @click="openReviewDeleteModal = true" class="block w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-100 dark:text-red-400 dark:hover:bg-red-800">
                                                Delete
                                            </button>
                                            @endcan

                                            <!-- Confirmation Modal -->
                                            <div x-show="openReviewDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                                                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-[400px]">
                                                    ...
                                                    <div class="flex justify-end space-x-3 mt-3">
                                                        <!-- Cancel Button -->
                                                        <button @click="openReviewDeleteModal = false" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                                                            Cancel
                                                        </button>

                                                        <!-- Delete Form -->
                                                        <form action="{{ route('be.review.destroy', $review->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
            ...
            <div class="{{ !$reviews->previousPageUrl() && !$reviews->nextPageUrl() ? '' : 'border-t border-gray-200 px-6 py-4 dark:border-gray-800' }}">
