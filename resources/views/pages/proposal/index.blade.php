   ...
   <div class="p-4 mx-auto max-w-screen-2xl md:p-6">
    ...
    <div class="flex px-6 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">Manage proposal data</p>
        </div>
        @can(PermissionEnum::CREATE_PROPOSAL, $proposals)
            <a href="{{ route('be.proposal.create') }}"
                            ...
                            <a href="#"
                                x-on:click.prevent="
                                    if (selected.length > 0) {
                                        ...
                                        deleteUrl = '{{ route('be.proposal.mass.destroy') }}?' + params.toString();
                        ...
                        <div x-data="{ open: false, selectedField: '{{ request()->query('filter') ? array_key_first(request('filter')) : '' }}' }">
                            ...
                            <div x-cloak x-show="open" @keydown.escape.window="open = false"
                                ...
                                <div @click.away="open = false"
                                    ...
                                    <!-- Form -->
                                    <form method="GET" action="{{ route('be.proposal.index') }}">
                                        <!-- Limit Selection -->
                                        <div class="mt-4">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-800 dark:text-gray-400">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                            <td class="w-10 px-6 py-3">
                                <input
                                    type="checkbox"
                                    class="proposal-checkbox flex h-5 w-5 border-gray-300 cursor-pointer items-center justify-center rounded-md border-[1.25px] transition-all" value="{{ $proposal->slug }}"
                                    x-model="selected">
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-col items-center">
                                    ...
                                    <span class="mt-2 text-xs font-medium text-gray-500 dark:text-gray-300">{{ $proposal->name }}</span>
                                </div>
                            </td>
                            <td class="w-20 px-4 py-3">{{ $proposal->title }}</td>
                            <td class="px-4 py-3">{{ $proposal->formatted_created_at }}</td>
                            <td class="px-4 py-3">{{ $proposal->formatted_updated_at }}</td>
                            <td class="px-4 py-3 text-center relative">
                                <div x-cloak x-data="{ openDropDown: false }" class="inline-block">
                                    ...
                                    <div x-show="openDropDown" @click.outside="openDropDown = false"
                                        ...
                                        @can(PermissionEnum::UPDATE_PROPOSAL, $proposal)
                                        <a href="{{ route('be.proposal.edit', $proposal->slug) }}" class="block w-full px-4 py-2 text-left text-sm text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800">
                                            Edit
                                        </a>
                                        @endcan
                                        ...
                                        <div x-data="{ openProposalDeleteModal: false }">
                                            <!-- Delete Button -->
                                            @can(PermissionEnum::DELETE_PROPOSAL, $proposal)
                                            <button @click="openProposalDeleteModal = true" class="block w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-100 dark:text-red-400 dark:hover:bg-red-800">
                                                Delete
                                            </button>
                                            @endcan

                                            <!-- Confirmation Modal -->
                                            <div x-show="openProposalDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                                                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-[400px]">
                                                    ...
                                                    <div class="flex justify-end space-x-3 mt-3">
                                                        <!-- Cancel Button -->
                                                        <button @click="openProposalDeleteModal = false" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                                                            Cancel
                                                        </button>

                                                        <!-- Delete Form -->
                                                        <form action="{{ route('be.proposal.destroy', $proposal->slug) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
